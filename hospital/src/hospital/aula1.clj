...
(defn simula-um-dia []
  ...
  ;funciona mas ja comecamos a ter um treco no caraco com
  ; esse simbolo global, com root binding sendo alterado a rodo ò-ó
  (pprint hospital)

  (def hospital (h.logic/chega-em hospital :laboratorio1 "444"))
  (def hospital (h.logic/chega-em hospital :laboratorio4 "555"))
  (pprint hospital)
...
(defn simula-um-dia-em-paralelo
  []
  ;Muito claro o problema de variavel global (simbolo do namespace) -
  ...
  (.start (Thread. (fn [] (chega-em-malvad  o "444"))))
  (.start (Thread. (fn [] (chega-em-malvado "555"))))
  (.start (Thread. (fn [] (chega-em-malvado "666"))))
  (.start (Thread. (fn [] (Thread/sleep 4000)
                     (pprint hospital)))))

;(.start (Thread. (fn [] (println "oi1"))))                ;. chama o new do java
